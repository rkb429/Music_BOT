...
내 플레이리스트 → 수록곡 아티스트 빈도 분석
재생 기록 DB → play_count 상위 아티스트 추출
기본 인기 차트 키워드 랜덤 선택 (위 두 단계 모두 실패 시)
...
config.json에서 볼륨, DJ 추천 수, FFmpeg 옵션 등을 조정할 수 있습니다.
...
비동기 yt-dlp — run_in_executor로 블로킹 추출을 스레드 풀에 위임, 이벤트 루프 차단 없음
Cog 간 통신 — bot.get_cog() 런타임 조회로 순환 임포트 방지
메모리 관리 — 1시간 주기 정리 태스크 + 길드 퇴장 시 즉시 _cleanup_guild_data()
설정 외부화 — ytdl/ffmpeg 옵션을 config.json으로 분리, cog 재시작 없이 조정 가능
