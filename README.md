...
- 네트워크 끊김 **자동 재연결** (`reconnect 1 -reconnect_streamed 1`)
...
- 모든 버튼 응답 **ephemeral** (클릭한 사람에게만 표시)
...
MusicCog ──bot.get_cog()──► DatabaseCog
  ...
  DJ 추천 → get_user_playlists(), get_play_history()
...
| `!입장` | `!노래봇` | 음성 채널 입장 |
| `!나가기` | `!ㅂㅂ`, `!leave` | 퇴장 및 상태 초기화 |
| `!스킵` | `!다음`, `!skip` | 현재 곡 건너뜀 |
| `!목록` | `!q`, `!queue` | 대기열 표시 |
| `!삭제 [번호]` | `!del`, `!remove` | 대기열 특정 곡 제거 |
| `!볼륨 [0-100]` | `!vol` | 볼륨 설정 |
| `!셔플` | `!shuffle` | 대기열 셔플 |
| `!반복` / `!전체반복` / `!반복종료` | `!loop` / `!loopall` / `!unloop` | 반복 모드 전환 |

### **DJ 모드**

| 명령어 | 설명 |
| --- | --- |
| `!자동` | DJ 모드 ON (대기열 10곡 즉시 채움) |
| `!자동끄기` | DJ 모드 OFF |
| `!추천` | 현재 곡 기반 추천 5곡 표시 |
...
명령어는 채널명에 다음 키워드가 포함된 채널에서만 동작합니다: `노래`, `음악`, `music`, `봇`, `bot`
...
| --- | --- |
| 언어 | Python 3.11+ |
| Discord | discord.py 2.x (Cog 아키텍처, Application Commands) |
